...
def review_to_int(reviewtexts):
    ...
    reviewtexts = [token for rev in tqdm(reviewtexts, desc="Creating integer token dictionary", leave=False) for token in rev]
...
def tokenize(review):
    ...
    for token in text:
        ...
        for i, op in enumerate(review.opinions):
            if start <= op.target_position[0] < end or start < op.target_position[1] <= end:
...
class PreprocessingPipeline(object):
    ...
    If this option is set, applying the pipeline will return a e-tuple of nested
    lists with tokens opinions and polarity, instead of the original reviews
    ...
    If this option is set, applying the pipeline will return a 5-tuple of
    transformed review text, opinions, sentiment and the forward and
    reverse dict for integer conversion"""
    def __init__(self, lower=True, rm_special_char=True, rm_stopwords=True, tokenize=True, stemming=False, rev_to_int=True):
        if rev_to_int or stemming:
            assert tokenize, "Need to tokenize before stemming or converting to int, but tokenize is set to False"
        self.lower = lower
    ...
    def __call__(self, reviews):
        if self.lower:
            reviews = list(map(lower, tqdm(reviews, desc="Converting to lowercase", leave=False)))
        if self.rm_special_char:
            reviews = list(map(remove_special_characters, tqdm(reviews, desc="Removing special characters", leave=False)))
        if self.rm_stopwords:
            reviews = list(map(remove_stopwords, tqdm(reviews, desc="Removing stopwords", leave=False)))
        if self.tokenize:
            reviews = list(map(tokenize, tqdm(reviews, desc="Tokenizing", leave=False)))
            reviews = ([rev[0] for rev in reviews], [rev[1] for rev in reviews], [rev[2] for rev in reviews])
        if self.stemming:
            reviews[0] = list(map(stemming, tqdm(reviews[0], desc="Stemming", leave=False)))
        if self.rev_to_int:
            str_to_int_dict, int_to_str_dict = review_to_int(reviews[0])
            return [[str_to_int_dict[token] for token in rev] for rev in tqdm(reviews[0], desc="Converting reviews to integer", leave=False)], reviews[1], reviews[2], str_to_int_dict, int_to_str_dict
        return reviews
