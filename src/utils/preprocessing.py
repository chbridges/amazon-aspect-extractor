...
def tokenize_review(review):
    ...
    for token in text:
        ...
        for i, op in enumerate(review.opinions):
            if op.target_position[0] <= start < op.target_position[1] or op.target_position[0] < end <= op.target_position[1]:
...
class PreprocessingPipeline(object):
    ...
    def __call__(self, dataset):
        ...
        for phase in tqdm(dataset.keys(), desc="Processing dataset splits", leave=False, position=0):
        ...
        if self.rev_to_int:
            combined_reviews = [rev for phase in dataset.keys() for rev in dataset[phase][0]]
            str_to_int_dict, int_to_str_dict = review_to_int(combined_reviews)
            for phase in tqdm(dataset.keys(), desc="Processing dataset splits", leave=False, position=0):
                reviews = dataset[phase]
                reviews = [[str_to_int_dict[token] for token in rev] for rev in tqdm(reviews[0], desc="Converting reviews to integer", leave=False, position=1)], reviews[1], reviews[2]
                dataset[phase] = reviews
