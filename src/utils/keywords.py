...
def extract_keywords_str(
    document: str, minWords: int = 1, maxKeywords: int = -1, minScore: float = 0.0, useNLTK: bool = True
) -> List[str]:
    ...
    keywords = [k for k in keywords if k[1] >= minScore and count_words(k[0]) >= minWords]
...
def keywords_to_dataframe(keywords: list, include_relevancy: bool = True, csv_name: str = ""):
