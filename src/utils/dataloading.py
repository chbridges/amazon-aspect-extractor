...
def load_amazon_multilingual(
    ...
    for split in os.listdir(path):
        ...
        for json_file in os.listdir(os.path.join(path, split)):
            if any(
                map(json_file.__contains__, select_languages)
...
def load_semeval2015(
    ...
    for filename in os.listdir(path):
        if filename.endswith(".xml") and any(
            ...
            for review in root:
                ...
                for sentence in review[0]:  # review[0] == sentences field
                    ...
                    for op in sentence[1]:  # sentence[1] == opinions
                        ...
                        assert (
                            len(category) == 2
                        ), "category has wrong format: {}".format(category)
                        if (
                            op.attrib.get("polarity") is None
                            or op.attrib.get("polarity") == ""
                        ):
                            continue
                        pos = (op.attrib.get("from"), op.attrib.get("to"))
                        opinion = SemEvalReviewOpinion(
                            op.attrib.get("target"),
...
class SemEvalReviewOpinion(object):
    ...
    def shift_pos(self, shift):
        if not self.target_position == (0, 0):
            self.target_position = (
                self.target_position[0] + shift,
                self.target_position[1] + shift,
            )
...
class SemEvalReview(object):
    ...
    def __init__(self, text: List[str], opinions: List):
        assert len(text) == len(
            opinions
        ), "Length of opinion and text list not matching: {} - {}".format(
            len(opinions), len(text)
        )
        self.text = ""
        self.opinions = []
    ...
    def remove_text(self, span):
        ...
        for i, op in enumerate(self.opinions):
            ...
            if start <= op_end:
                op_end = op_end - min(
                    end - start, op_end - start
                )  # end and op_end both exclusive
